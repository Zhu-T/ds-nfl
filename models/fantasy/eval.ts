...
const ADMITS_GAP =
  /\b(not|n't)\b[^.]{0,60}\b(include|cover|say|have|contain|mention|provide|list|available|know)|\bno (information|data|stats|statistics)\b|\bcan(not|'t) (tell|say|answer)/i;
...
function score(c: Case, text: string): Verdict {
  const flags: string[] = [];
  ...
  if (/\b(franchise|dynamic|reliable|explosive|game-changer|game-breaking|signal-caller|bell-?cow|elite|workhorse|proven)\b/i.test(text)) {
    flags.push('scouting claim');
  }
