...
describe('getFreeAgentPool', () => {
  it('asks for the best projected for the week, the most rostered and the most added, and merges them', async () => {
    const filters: any[] = [];
    vi.stubGlobal('fetch', async (_input: string | URL | Request, init?: RequestInit) => {
      const filter = JSON.parse((init?.headers as Record<string, string>)['x-fantasy-filter'] ?? '{}');
      filters.push(filter.players);
      const ids = filter.players.sortAppliedStatTotal ? [11, 12, 13] : filter.players.sortPercChanged ? [14, 15] : [13, 14];
      return new Response(JSON.stringify({ players: ids.map(entry) }));
    });

    const pool = await new EspnReader({ espnS2: 's', swid: '{w}' }).getFreeAgentPool(ref, 3, { byProjection: 3, byOwnership: 2, byTrend: 2 });
    ...
    expect(filters.find((f) => f.sortPercChanged)).toMatchObject({ limit: 2, sortPercChanged: { sortAsc: false } });
    expect(pool.map((p) => p.platformPlayerId)).toEqual(['11', '12', '13', '14', '15']);
  });
});
